...
本节直接回答设计说明中的三项要求，便于评审对照。

### 0.1 系统边界

**在本系统中选择解决的问题：**

- **异步解耦**：业务提交后立即返回，投递在后台执行，不阻塞调用方。
- **可靠投递**：SQLite 持久化，进程重启不丢任务。
- **失败重试**：单次 HTTP 投递失败时，由本系统按指数退避（`delay = 10s × 3^retry_count`）自动重试，重试次数有上限（可配置），既提高送达率，又避免对下游造成瞬时压力。
- **死信与人工介入**：当重试次数达到上限仍失败时，任务标记为 `failed`，视为死信；保留请求体、最近错误信息等，运维可通过「列出失败任务 + 单条重放」API 介入，重放后任务重新进入待投递队列。
- **认证与 Token**：内置 `bearer` / `api_key` / `basic` 凭证注入由本系统负责；复杂刷新或签名逻辑通过代码 Adapter 扩展，不要求调用方传入或维护有效凭证。

**明确选择不解决的问题及原因：**

| 不解决项 | 原因 |
|----------|------|
| 投递侧去重 / 接收端幂等 | 提交侧已用 `biz_id` 防重复创建 Job；同一 Job 可能因重试被多次投递，接收方需自行幂等，本系统不越界做接收端语义。 |
| 限流 / 频控 | MVP 阶段不做；可在演进阶段按需增加。 |
| 通知内容的业务校验 | 本系统只负责“按配置投递”，不解析或校验业务语义。 |

### 0.2 可靠性与失败处理

**投递语义：** **至少一次（At-least-once）**。同一 Job 可能因重试被多次投递到接收方，接入方必须能按 `biz_id`（或等价业务键）做幂等处理。

**外部系统失败或长期不可用时的处理策略：**

- **单次失败**：视为一次投递失败，按指数退避（`delay = 10s × 3^retry_count`）重试，不阻塞其他 Job、其他 Vendor。
- **多次失败未超限**：每次失败先判断 `retry_count + 1 >= max_retries`；若不满足则调用 `MarkRetry`，将任务回退为 `pending` 并递增 `retry_count`。
- **达到重试上限**：当 `retry_count + 1 >= max_retries` 时，直接标记为 `failed` 并记录 `last_error`。
- **长期不可用**：行为与“多次失败”一致——重试耗尽后进死信，不无限重试；运维可通过死信列表发现并处理（修好外部系统后手动重放）。
- **本进程崩溃**：`processing` 状态超时（默认 1 分钟）后由调度器回收，重新进入待投递队列，保证不丢任务。

### 0.3 取舍与演进

**认为“过度”而未采纳的设计：**

- **Gin 等第三方 HTTP 框架**：引入大量间接依赖，与“控制复杂度、少依赖”原则不符；`net/http` 已满足路由与封装需求。
- **纯 Go SQLite 驱动（如 modernc.org/sqlite）**：包体积与下载成本高，MVP 选用 CGo 版 `mattn/go-sqlite3`。
- **MVP 阶段做限流/频控、MQ、多实例**：首版聚焦单机可运行、逻辑正确与可观测性，上述能力留待演进。

**若为第一版系统，未来流量或复杂度显著增长时的演进方向：**

- **存储**：SQLite → MySQL/PostgreSQL（需新增对应 `store` 实现并在 `main.go` 替换注入）。
- **队列**：DB 轮询 → Redis / RabbitMQ 等，提升吞吐与解耦。
- **水平扩展**：当前 Worker 已用乐观锁抢占，多实例部署即可横向扩展。
- **可观测与运维**：增加 Prometheus 指标、Grafana、限流、管理界面等。

上述演进在本文档 [§13 未来演进](#13-未来演进) 中有具体路线说明。
...
| 可靠投递 | SQLite 持久化，进程重启不丢任务 |
| 失败重试 | 指数退避策略（`delay = 10s × 3^retry_count`）；默认 `max_retries=3` 时会在 10s、30s 后进入 `failed` |
...
| 校验项 | 规则 |
|--------|------|
| 请求体大小（创建/更新） | ≤ 1MB (`MaxBytesReader`) |
| JSON 格式（创建/更新） | 禁止未知字段 (`DisallowUnknownFields`) |
| 单对象（创建/更新） | 请求体只能包含一个 JSON 对象 |
| id | 创建时必填，不可更新 |
| name | 必填 |
...
if retry_count + 1 >= max_retries:
    ...
    MarkRetry(next_retry_at = now + BackoffDuration(retry_count))
```

| 场景 | 结果 |
|------|------|
| 默认 `max_retries=3` | 失败后等待 10s、30s；第三次失败直接标记 `failed` |
| `max_retries>=4` | 在上述基础上可继续出现 90s、270s ... 的退避等待 |
...
| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `DB_PATH` | `notifications.db` | SQLite 数据库文件路径；相对路径会基于当前工作目录解析并自动创建父目录 |
| `ADDR` | `:8080` | HTTP 监听地址 |
...
| 层 | 策略 |
|----|------|
| **handler** | 统一使用 `Response` 信封返回（`respondSuccess` / `respondError` / `respondList`），`code=0` 表示成功，非零为 HTTP 状态码；创建通知时 Vendor 不存在返回 `400`；重复提交通知返回 `200` |
